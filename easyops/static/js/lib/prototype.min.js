...
Ajax.Response = Class.create({
  ...
  _getResponseJSON: function () {
    ...
    if (
      !a.evalJSON ||
      (a.evalJSON != "force" && !(this.getHeader("Content-type") || "").include("application/json")) ||
...
(function (a8) {
  ...
  Object.extend(a3, { setStyle: C, getStyle: aO, setOpacity: z, getOpacity: bd });
...
(function (av) {
  var D,
    ...
    ah = {
      ...
      needsContext: new RegExp(
        "^" + v + "*[>+~]|:(even|odd|eq|gt|lt|nth|first|last)(?:\\(" + v + "*((?:-\\d)?\\d*)" + v + "*\\)|)(?=[^-]|$)",
        "i",
      ),
    },
  ...
  ae = B.setDocument = function (aD) {
    ...
    if (
      (ay.matchesSelector = U.test(
        (k = u.webkitMatchesSelector || u.mozMatchesSelector || u.oMatchesSelector || u.msMatchesSelector),
      ))
    ) {
      l(function (aF) {
        ...
        ? function (aG, aF) {
            ...
            return (
              ...
              !!(
                aH &&
                aH.nodeType === 1 &&
                (aI.contains ? aI.contains(aH) : aG.compareDocumentPosition && aG.compareDocumentPosition(aH) & 16)
              )
            );
          }
        : function (aG, aF) {
  ...
  t = B.selectors = {
    ...
    filter: {
      ...
      PSEUDO: function (aE, aD) {
        var e,
          i = t.pseudos[aE] || t.setFilters[aE.toLowerCase()] || B.error("unsupported pseudo: " + aE);
        if (i[ap]) {
          return i(aD);
        }
        if (i.length > 1) {
  ...
  for (D in { radio: true, checkbox: true, file: true, password: true, image: true }) {
    t.pseudos[D] = C(D);
  }
    ...
    for (; aF < aH; aF++) {
      if ((aG = t.relative[aI[aF].type])) {
        ...
        if (aG[ap]) {
          ...
          return m(
            aF > 1 && aB(e),
            aF > 1 && o(aI.slice(0, aF - 1).concat({ value: aI[aF - 2].type === " " ? "*" : "" })).replace(x, "$1"),
            aG,
    ...
    return h.inject(c, function (i, j) {
      if (!j.disabled && j.name) {
        ...
        if (
          g != null &&
          j.type != "file" &&
          (j.type != "submit" || (!a && f !== false && (!f || e == f) && (a = true)))
        ) {
          i = b(i, e, g);
        }
      }
...
Abstract.TimedObserver = Class.create(PeriodicalExecuter, {
  ...
  execute: function () {
    var a = this.getValue();
    if (
      Object.isString(this.lastValue) && Object.isString(a) ? this.lastValue != a : String(this.lastValue) != String(a)
    ) {
  ...
  function K(Z) {
    ...
    if (aa && aa.tagName) {
      if (
        X === "load" ||
        X === "error" ||
        (X === "click" && aa.tagName.toLowerCase() === "input" && aa.type === "radio")
      ) {
        Y = aa;
      }
    }
